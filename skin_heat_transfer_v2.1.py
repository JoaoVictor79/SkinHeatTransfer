...
N *= 116
Tt = 1800  # Tempo total [s]
n = 1000  # Número de divisões do tempo
...
fig, ax = plt.subplots()
mesh = ax.pcolormesh(nos, np.arange(10), temp_map, shading='auto', cmap='jet', vmin=30, vmax=37)
fig.colorbar(mesh, label="Temperatura (°C)")
ax.set_title("Transferência de calor na pele")
ax.set_xlabel("Distância (m)")
ax.set_yticks([])
rotulos = ['Epiderme', 'Derme papilar', 'Derme reticular', 'Tecido adiposo subcutâneo', 'Camada muscular']
for i in range(1,6):
    ax.axvline(x=interfaces[i]*dx, color='white', linestyle='--')
    meio = interfaces[i-1] + (interfaces[i] - interfaces[i-1]) / 2
    ax.text(meio*dx, 5, rotulos[i-1], rotation=90, verticalalignment='center', horizontalalignment='center', color='white', fontsize=15)
ax.axvline(x=pi, color='black', linestyle='-')
ax.axvline(x=pf, color='black', linestyle='-')
ax.text((pf+pi)/2, 1, 'Tumor', verticalalignment='center', horizontalalignment='center', color='black', fontsize=15)


# Função atualização de quadros
def update(frame):
    temperaturas = np.linalg.solve(A,B)
    popB(temperaturas)
    temp_map = np.tile(temperaturas, (10,1))
    for coll in ax.collections:
        coll.remove()
    ax.pcolormesh(nos, np.arange(10), temp_map, shading='auto', cmap='jet',vmin=30, vmax=37)
    return ax.collections

ani = FuncAnimation(fig, update, frames=range(500), interval=500)
plt.show()